...
fname<-"SLC_Line_pac_clip_cut.shp" #creates the file name variable that is then used to read in the shape file via readOGR
...
for (i in 10:10){
  shape@data[length(shape@data)+1]<-as.numeric((shape@data[length(shape@data)-1]^(1/2))) #transform data to reduce left skew
  ...
  alt<-c(min(shape@data[,length(shape@data)-2],na.rm=T),lev4)
  aln<-(alt^2)
  lim<-as.data.frame(cbind(aln,alt))
  ...
  hist(shape@data[,length(shape@data)-4], main="Histogram of Mean SLC", xlab="Sea-level Distribution (m)")
  abline(v=aln,lty=2,col="blue")
  hist(shape@data[,length(shape@data)-2], main="Histogram of Transformed Data", xlab="Sea-level Distribution (original value ^ 1/2)")
