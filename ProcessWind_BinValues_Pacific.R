...
fname<-"Windspeed_Line_pac_clip" #creates the file name variable that is then used to read in the shape file via readOGR
...
for (i in 3:4){
  ...
  shape@data[length(shape@data)+1]<-as.numeric(shape@data[i]^(3.4))
  ...
  alt<-(c(min(shape@data[length(shape@data)-2]),lev4))
  aln<-(alt^(1/3.4))
  ...
  hist(shape@data[,i], main="Histogram of Original Data",xlab="Mean Maximum Wind Speed (m/s)", xlim=c(20,33))
  abline(v=aln,lty=2,col="blue")
  hist(shape@data[,length(shape@data)-2], main="Histogram of Transformed Data",xlab="Transformed Mean Maximum Wind Speed (^3.4)")
...
write.csv(lim,"WS_LINE_PACIFIC_CLIP_CUTS.csv")
